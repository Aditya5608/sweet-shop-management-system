...
def create_sweet_endpoint(sweet: schemas.SweetCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    return crud.create_sweet(db, sweet)

@router.get("", response_model=List[schemas.SweetRead])
def list_sweets(db: Session = Depends(get_db), skip: int = 0, limit: int = 100):
...
def search_sweets(name: Optional[str] = None, category: Optional[str] = None,
    ...
    q = db.query(crud.__import__("app.models").Sweet)  # fallback; we won't use this heavily in tests
...
def update_sweet(sweet_id: int, sweet: schemas.SweetUpdate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
...
def purchase(sweet_id: int, qty: int = Query(1, ge=1), db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    s = crud.purchase_sweet(db, sweet_id, qty)
    if not s:
        raise HTTPException(status_code=400, detail="Insufficient stock or sweet not found")
    return s

@router.post("/{sweet_id}/restock", response_model=schemas.SweetRead)
def restock(sweet_id: int, qty: int = Query(..., ge=1), db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    if not current_user.is_admin:
        raise HTTPException(status_code=403, detail="Admin only")
